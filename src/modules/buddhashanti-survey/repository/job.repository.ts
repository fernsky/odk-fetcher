...
export class JobRepositoryImpl implements JobRepository {
  async createJob(type: string): Promise<Job> {
    ...
    const [job] = await buddhashantiDb
      .insert(jobModel)
      .values({
        id: jobId,
        type: type,
  ...
  async getActiveJob(type: string): Promise<Job | null> {
    const jobs = await buddhashantiDb
      .select()
      .from(jobModel)
      .where(
        and(
  ...
  async updateJobStatus(jobId: string, status: JobStatus): Promise<Job> {
    const [updatedJob] = await buddhashantiDb
      .update(jobModel)
      .set({
        status: status,
      })
  ...
  async completeJob(jobId: string): Promise<Job> {
    const [completedJob] = await buddhashantiDb
      .update(jobModel)
      .set({
  ...
  async failJob(jobId: string): Promise<Job> {
    const [failedJob] = await buddhashantiDb
      .update(jobModel)
      .set({
