...
export class BuildingSurveyRepositoryImpl implements BuildingSurveyRepository {
  ...
  async findBuildingSurveys(
    ...
    // Create the query
    const query = duduwaDb
      .select()
      .from(surveyData)
      .where(
        cursor
          ? and(
              eq(surveyData.formId, 'duduwa_building_survey'),
  ...
  async findFamilySurveys(): Promise<SurveyData<RawFamily>[]> {
    ...
    const surveys = await duduwaDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'duduwa_family_survey'))
  ...
  async findBusinessSurveys(): Promise<SurveyData<RawBusiness>[]> {
    ...
    const surveys = await duduwaDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'duduwa_business_survey'))
      .orderBy(surveyData.created_at);
