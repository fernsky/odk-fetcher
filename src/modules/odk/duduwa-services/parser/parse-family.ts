  ...
  try {
    ...
    if (r.individual?.length > 0) {
      for (const i of r.individual) {
        try {
          // Process individual
          const individual = await processIndividual(i, r);

          // Insert individual
          try {
            const individualStatement = jsonToPostgres(
              'staging_duduwa_individual',
              individual,
...
function processIndividual(i: any, r: RawFamily) {
  try {
    const individual = {
      id: i.__id,
      household_id: r.__id,
