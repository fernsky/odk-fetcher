...
import {
  ...
  users,
  wards,
} from '@app/modules/drizzle/duduwa-db/schema';
import {
  family,
  stagingFamily,
} from '@app/modules/drizzle/duduwa-db/schema/family/family';
import {
  stagingduduwaIndividual,
  duduwaIndividual,
} from '@app/modules/drizzle/duduwa-db/schema/family/individual';
import {
  stagingduduwaDeath,
  duduwaDeath,
} from '@app/modules/drizzle/duduwa-db/schema/family/deaths';
import {
  stagingduduwaCrop,
  duduwaCrop,
} from '@app/modules/drizzle/duduwa-db/schema/family/crops';
import {
  stagingduduwaAnimal,
  duduwaAnimal,
} from '@app/modules/drizzle/duduwa-db/schema/family/animals';
import {
  stagingduduwaAnimalProduct,
  duduwaAnimalProduct,
} from '@app/modules/drizzle/duduwa-db/schema/family/animal-products';

import { StagingduduwaDeath } from '@app/modules/drizzle/duduwa-db/schema/family/deaths';
import duduwaAgriculturalLand, {
  stagingduduwaAgriculturalLand,
} from '@app/modules/drizzle/duduwa-db/schema/family/agricultural-lands';
...
async function performFamilySync(ctx: any, recordId: string) {
  try {
    ...
    const individuals = await ctx.db
      .select()
      .from(stagingduduwaIndividual)
      .where(eq(stagingduduwaIndividual.familyId, recordId));

    const agriculturalLands = await ctx.db
      .select()
      .from(stagingduduwaAgriculturalLand)
      .where(eq(stagingduduwaAgriculturalLand.familyId, recordId));

    const deaths = await ctx.db
      .select()
      .from(stagingduduwaDeath)
      .where(eq(stagingduduwaDeath.familyId, recordId));

    const crops = await ctx.db
      .select()
      .from(stagingduduwaCrop)
      .where(eq(stagingduduwaCrop.familyId, recordId));

    const animals = await ctx.db
      .select()
      .from(stagingduduwaAnimal)
      .where(eq(stagingduduwaAnimal.familyId, recordId));

    const animalProducts = await ctx.db
      .select()
      .from(stagingduduwaAnimalProduct)
    ...
    await ctx.db.transaction(async (tx: any) => {
      ...
      if (deaths.length > 0) {
        await tx
          .insert(duduwaDeath)
          .values(
            deaths.map((death: StagingduduwaDeath) => ({
              id: death.id,
      ...
      // Insert crops
      if (crops.length > 0) {
        await tx.insert(duduwaCrop).values(crops).onConflictDoNothing();
      }

      // Insert animals
      if (animals.length > 0) {
        await tx.insert(duduwaAnimal).values(animals).onConflictDoNothing();
      }

      // Insert animal products
      if (animalProducts.length > 0) {
        await tx
          .insert(duduwaAnimalProduct)
      ...
      await tx
        ...
        .values({
          staging_table: 'staging_duduwa_family',
          production_table: 'duduwa_family',
          recordId: recordId,
        })
        .onConflictDoNothing();
    });
