...
import { gadhawaDb } from '../../../modules/drizzle/gadhawa-db';
  ...
  implements BuildingAggregateRepository
{
  ...
  async saveAggregateBuilding(
    buildingData: any | NewgadhawaAggregateBuilding,
  ): Promise<void> {
    try {
      this.logger.debug(
      ...
      const statement = jsonToPostgres('gadhawa_aggregate_buildings', payload);

      if (statement) {
        this.logger.debug('Executing SQL insert statement');
        // Execute the generated SQL statement
        await gadhawaDb.execute(sql.raw(statement));
        this.logger.log(
  ...
  async findByBuildingToken(buildingToken: string): Promise<any | null> {
    try {
      ...
      const results = await gadhawaDb
        .select()
        .from(gadhawaAggregateBuilding)
        .where(eq(gadhawaAggregateBuilding.building_token, buildingToken))
        .limit(1);
  ...
  async findById(id: string): Promise<gadhawaAggregateBuilding | null> {
    try {
