...
export class BuildingSurveyRepositoryImpl implements BuildingSurveyRepository {
  ...
  async findBuildingSurveys(
    ...
    // Create the query
    const query = gadhawaDb
      .select()
      .from(surveyData)
      .where(
        cursor
          ? and(
              eq(surveyData.formId, 'gadhawa_building_survey'),
  ...
  async findFamilySurveys(): Promise<SurveyData<RawFamily>[]> {
    ...
    const surveys = await gadhawaDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'gadhawa_family_survey'))
  ...
  async findBusinessSurveys(): Promise<SurveyData<RawBusiness>[]> {
    ...
    const surveys = await gadhawaDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'gadhawa_business_survey'))
      .orderBy(surveyData.created_at);
