  ...
  implements BuildingAggregateRepository
{
  ...
  async saveAggregateBuilding(
    buildingData: any | NewlungriAggregateBuilding,
  ): Promise<void> {
    try {
      this.logger.debug(
      ...
      const statement = jsonToPostgres('lungri_aggregate_buildings', payload);

      if (statement) {
        this.logger.debug('Executing SQL insert statement');
        // Execute the generated SQL statement
        await lungriDb.execute(sql.raw(statement));
        this.logger.log(
  ...
  async findByBuildingToken(buildingToken: string): Promise<any | null> {
    try {
      ...
      const results = await lungriDb
        .select()
        .from(lungriAggregateBuilding)
        .where(eq(lungriAggregateBuilding.building_token, buildingToken))
        .limit(1);
  ...
  async findById(id: string): Promise<lungriAggregateBuilding | null> {
    try {
