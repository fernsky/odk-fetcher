...
export class BuildingSurveyRepositoryImpl implements BuildingSurveyRepository {
  ...
  async findBuildingSurveys(
    ...
    // Create the query
    const query = lungriDb
      .select()
      .from(surveyData)
      .where(
        cursor
          ? and(
              eq(surveyData.formId, 'lungri_building_survey'),
  ...
  async findFamilySurveys(): Promise<SurveyData<RawFamily>[]> {
    ...
    const surveys = await lungriDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'lungri_family_survey'))
  ...
  async findBusinessSurveys(): Promise<SurveyData<RawBusiness>[]> {
    ...
    const surveys = await lungriDb
      .select()
      .from(surveyData)
      .where(eq(surveyData.formId, 'lungri_business_survey'))
      .orderBy(surveyData.created_at);
